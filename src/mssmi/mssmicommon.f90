...
module mssmicommon
  ...
  public mssmi_beta, gmssmi_gen_norm_potential, gmssmi_gen_norm_deriv_potential, gmssmi_gen_norm_deriv_second_potential
...
!returns V/M**4
  function gmssmi_gen_norm_potential(x,alpha,Genbeta)
    implicit none
    real(kp) :: gmssmi_gen_norm_potential
    real(kp), intent(in) :: x,alpha
    real(kp), intent(in), optional :: Genbeta
    real(kp) :: beta

    if (present(Genbeta)) then
       beta=Genbeta
    else
       beta=mssmi_beta(alpha)
    endif
...
!returns the first derivative of the potential with respect to x, divided by M**4
  function gmssmi_gen_norm_deriv_potential(x,alpha,Genbeta)
    implicit none
...
!returns the second derivative of the potential with respect to x, divided by M**4
  function gmssmi_gen_norm_deriv_second_potential(x,alpha,Genbeta)
    implicit none
...
!epsilon_one(x)
  function gmssmi_gen_epsilon_one(x,alpha,Genbeta)    
    implicit none
    ...
    gmssmi_gen_epsilon_one =(2._kp*(1._kp-3._kp*alpha*x**4+5._kp*beta*x**8)**2)/(x-alpha*x**5+beta*x**9)**2
    
  end function gmssmi_gen_epsilon_one


!epsilon_two(x)
  function gmssmi_gen_epsilon_two(x,alpha,Genbeta)    
    implicit none
...
!epsilon_three(x)
  function gmssmi_gen_epsilon_three(x,alpha,Genbeta)    
    implicit none
...
!Returns the position of the first local minimum of epsilon1
  function gmssmi_gen_x_epsilon1_min(alpha,Genbeta)   
    implicit none
	...
	gmssmi_gen_x_epsilon1_min = (3._kp*alpha/(10._kp*beta)*(1._kp-sqrt(1._kp-20._kp*beta/(9._kp*alpha**2))))**(0.25_kp)
...
!returns x at the end of inflation defined as epsilon1=1
  function gmssmi_gen_x_endinf(alpha,Genbeta)
    implicit none
    real(kp), intent(in) :: alpha
    real(kp), intent(in), optional :: Genbeta
    real(kp) :: beta
