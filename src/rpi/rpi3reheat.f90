...
module rpi3reheat
  ...
  use rpi3sr, only : rpi3_ln_omega4
  ...
  public rpi3_x_rrad, rpi3_x_rreh, rpi3_lnrho_endinf

contains

!returns y =phi/Mp * sqrt(2/3) such given potential parameters, scalar power, wreh and
!lnrhoreh. If present, returns the corresponding bfoldstar
  function rpi3_x_star(p,yend,w,lnRhoReh,Pstar,bfoldstar)    
    implicit none
    real(kp) :: rpi3_x_star
...
!    lnOmega4End = 2._kp*yEnd
    lnOmega4End = rpi3_ln_omega4(yEnd,p)
...
!If present, returns the corresponding bfoldstar
  function rpi3_x_rrad(p,yend,lnRrad,Pstar,bfoldstar)    
    implicit none
    real(kp) :: rpi3_x_rrad
...
!If present, returns the corresponding bfoldstar
  function rpi3_x_rreh(p,yend,lnRreh,bfoldstar)    
    implicit none
    real(kp) :: rpi3_x_rreh
...
!consistency check
  ...
  function rpi3_lnrhoreh_max(p,yend,Pstar) 
    implicit none
...
!   Trick to return y such that rho_reh=rho_end
    ...
    y = rpi3_x_star(p,yend,wrad,junk,Pstar)  
...
!    lnOmega4End = 2._kp*yEnd
  ...
  function rpi3_lnrho_endinf(p,yend,ystar,Pstar)
    implicit none
    real(kp) :: rpi3_lnrho_endinf
    real(kp), intent(in) :: p,yend,ystar,Pstar
    ...
    epsOneEnd = rpi3_epsilon_one(yend,p)
    lnOmega4End = rpi3_ln_omega4(yend,p)
    potStar = rpi3_norm_potential(ystar,p)
    epsOneStar = rpi3_epsilon_one(ystar,p)

    rpi3_lnrho_endinf = ln_rho_endinf(Pstar,epsOneStar,epsOneEnd,potEnd/potStar,lnOmega4End)

  end function rpi3_lnrho_endinf
