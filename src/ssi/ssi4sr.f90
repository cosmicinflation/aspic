...
!returns x at the end of inflation defined as epsilon1=1
  function ssi4_x_endinf(alpha,beta)
    ...
    ssi4_x_endinf = zbrent(find_ssi4_x_endinf,mini,maxi,tolFind,ssi4Data)

  end function ssi4_x_endinf



  function find_ssi4_x_endinf(x,ssi4Data)    
    implicit none
    real(kp), intent(in) :: x   
    type(transfert), optional, intent(inout) :: ssi4Data
    real(kp) :: find_ssi4_x_endinf
    real(kp) :: alpha,beta

    alpha = ssi4Data%real1
    beta = ssi4Data%real2
    
    find_ssi4_x_endinf = ssi4_epsilon_one(x,alpha,beta) - 1._kp
   
  end function find_ssi4_x_endinf
...
!returns x at bfold=-efolds before the end of inflation, ie N-Nend
  function ssi4_x_trajectory(bfold,xend,alpha,beta)
    ...
    ssi4_x_trajectory = zbrent(find_ssi_x_trajectory,mini,maxi,tolFind,ssi4Data)
