...
module lmi1sr
  ...
  public  lmi1_beta,lmi1_alpha
 
contains

!Build the model parameters from beta consistently with the model ansatz a(t)=exp[A(ln(t)^lambda)]
  function lmi1_alpha(gamma_lmi,M)
    implicit none
    real(kp) :: lmi1_alpha
    real(kp), intent(in) :: gamma_lmi,M

    lmi1_alpha=4._kp*(1._kp-gamma_lmi/2._kp)

  end function lmi1_alpha

  function lmi1_beta(gamma_lmi,M)
    implicit none
    real(kp) :: lmi1_beta
    real(kp), intent(in) :: gamma_lmi,M

    lmi1_beta=sqrt(2._kp*sqrt(3._kp))/(M*gamma_lmi)

  end function lmi1_beta


!returns V/M^4
  function lmi1_norm_potential(x,gamma_lmi,M)
    implicit none
    ...
    lmi1_norm_potential = x**alpha*exp(-beta*x**gamma_lmi)
...
!returns the first derivative of the potential with respect to x, divided by M^4
  function lmi1_norm_deriv_potential(x,gamma_lmi,M)
    implicit none
    ...
    lmi1_norm_deriv_potential = exp(-beta*x**gamma_lmi)*x**(-1._kp+alpha)* &
...
!returns the second derivative of the potential with respect to x, divided by M^4
  function lmi1_norm_deriv_second_potential(x,gamma_lmi,M)
    implicit none
    ...
    lmi1_norm_deriv_second_potential = exp(-beta*x**gamma_lmi)*x**(-2._kp &
...
!epsilon_one(x)
  function lmi1_epsilon_one(x,gamma_lmi,M)    
    implicit none
...
!epsilon_two(x)
  function lmi1_epsilon_two(x,gamma_lmi,M)    
    implicit none
...
!epsilon_three(x)
  function lmi1_epsilon_three(x,gamma_lmi,M)    
    implicit none
...
!returns x at the end of inflation defined as epsilon1=1
  function lmi1_x_endinf(gamma_lmi,M)
    implicit none
    real(kp), intent(in) :: gamma_lmi,M
  ...
  function find_lmi1endinf(x,lmi1Data)    
    ...
    find_lmi1endinf = lmi1_epsilon_one(x,gamma_lmi,M) - 1._kp
...
!this is integral[V(phi)/V'(phi) dphi]
  function lmi1_efold_primitive(x,gamma_lmi,M)
    implicit none
    real(kp), intent(in) :: x,gamma_lmi,M
    ...
    if (alpha.eq.0._kp) stop 'lmi1_efold_primitive: alpha=0!'
...
!returns x at bfold=-efolds before the end of inflation, ie N-Nend
  function lmi1_x_trajectory(bfold,xend,gamma_lmi,M)
    implicit none
    ...
    maxi = (alpha/(beta*gamma_lmi))**(1._kp/gamma_lmi)*(1._kp-epsilon(1._kp))
    mini = lmi1_x_endinf(gamma_lmi,M)*(1._kp+epsilon(1._kp))
  

    lmi1Data%real1 = gamma_lmi
    lmi1Data%real2 = M
    lmi1Data%real3 = -bfold + lmi1_efold_primitive(xend,gamma_lmi,M)
  ...
  function find_lmi1traj(x,lmi1Data)    
    ...
    find_lmi1traj = lmi1_efold_primitive(x,gamma_lmi,M) - NplusNuend
