...
!this is integral[V(phi)/V'(phi) dphi]
  function rclfi3_efold_primitive(x,p,alpha,mu)
    implicit none
    real(kp), intent(in) :: x,p,alpha,mu
    real(kp) :: rclfi3_efold_primitive, epsone
...
#ifndef NOVC
    epsone = rclfi3_epsilon_one(x,p,alpha,mu)
    rclfi3_efold_primitive = rclfi3_efold_primitive + efold_velocity_correction((/epsone/))
#endif
