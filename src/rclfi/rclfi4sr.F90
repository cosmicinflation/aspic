...
!this is integral[V(phi)/V'(phi) dphi]
  function rclfi4_efold_primitive(x,p,alpha,mu)
    implicit none
    real(kp), intent(in) :: x,p,alpha,mu
    real(kp) :: rclfi4_efold_primitive, epsone
...
#ifndef NOVC
    epsone = rclfi4_epsilon_one(x,p,alpha,mu)
    rclfi4_efold_primitive = rclfi4_efold_primitive + efold_velocity_correction((/epsone/))
#endif    
...
!returns x at bfold=-efolds before the end of inflation, ie N-Nend
  function rclfi4_x_trajectory(bfold,xend,p,alpha,mu)
    ...
    rclfiData%msg = 'rclfi4_x_trajectory'
