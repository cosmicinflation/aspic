...
!lnrhoreh. If present, returns the corresponding bfoldstar
  function rclfi4_x_star(p,alpha,mu,xend,w,lnRhoReh,Pstar,bfoldstar)    
    implicit none
    ...
    primEnd = rclfi4_efold_primitive(xEnd,p,alpha,mu) 
    ...
    if (present(bfoldstar)) then
       x = rclfi4_x_star
       bfoldstar = - (rclfi4_efold_primitive(x,p,alpha,mu) - primEnd)
    endif
  ...
  function find_rclfi4_x_star(x,rclfiData)   
    ...
    primStar = rclfi4_efold_primitive(x,p,alpha,mu)
    epsOneStar = rclfi4_epsilon_one(x,p,alpha,mu)
    potStar = rclfi4_norm_potential(x,p,alpha,mu)
...
!If present, returns the corresponding bfoldstar
  function rclfi4_x_rrad(p,alpha,mu,xend,lnRrad,Pstar,bfoldstar)    
    implicit none
...
!If present, returns the corresponding bfoldstar
  function rclfi4_x_rreh(p,alpha,mu,xend,lnRreh,bfoldstar)    
    implicit none
  ...
  function rclfi4_lnrhoreh_max(p,alpha,mu,xend,Pstar) 
    implicit none
...
!   Trick to return x such that rho_reh=rho_end
    ...
    x = rclfi4_x_star(p,alpha,mu,xend,wrad,junk,Pstar)    
