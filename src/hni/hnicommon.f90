...
!returns the two solutions of epsilon1=1 in [0,pi]
  function hni_x_epsoneunity(alpha,f)
    ...
    if (alpha.lt.hni_alphamin(f)) then
...
!this is integral[V(phi)/V'(phi) dphi]
  function hni_efold_primitive(x,alpha,f)
    ...
    hni_efold_primitive = -f**2/alpha*(log(tan(x/2._kp))+alpha*log(sin(x)))
...
!Minimum value of alpha (depending on f) so that inflation ends naturally
  function hni_alphamin(f)
    implicit none
    real(kp) :: hni_alphamin
    real(kp), intent(in) :: f

    hni_alphamin = 1._kp/sqrt(1._kp+0.5_kp/f**2)

  end function hni_alphamin

!Maximum value of f (depending on alpha) so that inflation ends naturally
  function hni_fmax(alpha)
    implicit none
    real(kp) :: hni_fmax
    real(kp), intent(in) :: alpha

    hni_fmax = alpha/(sqrt(2._kp*(1._kp - alpha**2)))

  end function hni_fmax
