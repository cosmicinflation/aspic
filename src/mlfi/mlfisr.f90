...
module mlfisr
  ...
  public  mlfi_norm_deriv_potential, mlfi_norm_deriv_second_potential
...
!returns V/M^4
  function mlfi_norm_potential(x,p,q,alpha)
    ...
    mlfi_norm_potential = x**p*(1._kp+alpha*x**q)

  end function mlfi_norm_potential


!returns the first derivative of the potential with respect to x, divided by M^4
  function mlfi_norm_deriv_potential(x,p,q,alpha)
    implicit none
    real(kp) :: mlfi_norm_deriv_potential
    real(kp), intent(in) :: x,p,q,alpha

    mlfi_norm_deriv_potential = p*x**(p-1._kp) &
         + alpha*(p+q)*x**(p+q-1._kp)

  end function mlfi_norm_deriv_potential



!returns the second derivative of the potential with respect to x, divided by M^4
  function mlfi_norm_deriv_second_potential(x,p,q,alpha)
    implicit none
    real(kp) :: mlfi_norm_deriv_second_potential
    real(kp), intent(in) :: x,p,q,alpha

    mlfi_norm_deriv_second_potential = p*(p-1._kp)*x**(p-2._kp) &
         + alpha*(p+q)*(p+q-1._kp)*x**(p+q-2._kp)

  end function mlfi_norm_deriv_second_potential
...
!epsilon_one(x)
  function mlfi_epsilon_one(x,p,q,alpha)    
    ...
    mlfi_epsilon_one = 0.5_kp/(x**2) &
         *((p+alpha*(p+q)*x**q)/(1._kp+alpha*x**q))**2
...
!epsilon_two(x)
  function mlfi_epsilon_two(x,p,q,alpha)
    ...
    mlfi_epsilon_two = 2_kp/(x**2) &
        *(p+alpha**2*(p+q)*x**(2._kp*q)+alpha*(2._kp*p+q-q**2)*x**q) &
         /(1._kp+alpha*x**q)**2  
...
!epsilon_three(x)
  function mlfi_epsilon_three(x,p,q,alpha)
    ...
    mlfi_epsilon_three = 1._kp/(x**2) &
         *1._kp/(p*q**2+alpha**2*q**2*(p+q)*x**(2*q)+alpha*q**2*(2._kp*p+q-q**2)*x**q) &
         *1._kp/(1._kp+alpha*x**q)**2 &
         *(2._kp*p**2*q**2+2._kp*alpha**4*q**4*(1._kp+p/q)**2*x**(4._kp*q) &
         +alpha**2*q**2*(12._kp**2+6._kp*p*q*(2._kp-q)+(q-2._kp)*(q-1._kp)*q**2)*x**(2._kp*q) &
         +alpha**3*q**3*(p+q)*(8._kp*p/q+(1._kp-q)*(4._kp+q))*x**(3._kp*q) &
         +alpha*p*q**2*(8._kp*p+q*(4._kp+q**2-3._kp*q))*x**q)
...
!returns x at the end of inflation defined as epsilon1=1
  function mlfi_x_endinf(p,q,alpha)
    ...
    real(kp), parameter :: tomlfind=tolkp
    real(kp) :: mini,maxi
    ...
    mlfi_x_endinf = zbrent(find_mlfiendinf,mini,maxi,tomlfind,mlfiData)
  ...
  function find_mlfiendinf(x,mlfiData)
    ...
    find_mlfiendinf = x*sqrt(2._kp)*(1._kp+alpha*x**q) &
         -(p+alpha*(p+q)*x**q)
...
!returns x at bfold=-efolds before the end of inflation, ie N-Nend
  function mlfi_x_trajectory(bfold,xend,p,q,alpha)
    ...
    mlfi_x_trajectory = zbrent(find_mlfitraj,mini,maxi,tomlfind,mlfiData)
